...
class BancoSupabase:
    ...
    def __init__(self, url, chave, bucket=None):
        ...
        # Aceita a URL com ou sem sufixo: se vier ".../rest/v1" ou ".../storage/v1",
        # corta aqui. Sem isso o caminho saia duplicado (rest/v1/rest/v1) e dava 404.
        url = url.strip().rstrip("/")
        for _sufixo in ("/rest/v1", "/storage/v1", "/auth/v1"):
            if url.endswith(_sufixo):
                url = url[: -len(_sufixo)]
        self.url = url.rstrip("/")
        self.chave = chave
        self.bucket = bucket
